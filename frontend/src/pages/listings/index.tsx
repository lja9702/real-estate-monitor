...
export function ListingsPage() {
  ...
  // 모바일에선 필터를 기본 접어두고 토글(데스크탑은 항상 펼침).
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="sticky top-14 z-20 -mx-4 border-b bg-background/95 px-4 pt-3 pb-2 backdrop-blur supports-[backdrop-filter]:bg-background/75">
        {/* 모바일 필터 토글 바 — 항상 보이고, 결과 수를 함께 노출 */}
        <div className="flex items-center justify-between gap-3 md:hidden">
          ...
          <span className="text-sm text-muted-foreground">
            ...
            {!!data?.new_count && ` · 신규 ${data.new_count}`}
            {listingsQuery.isFetching && ' · …'}
          </span>
        </div>

        {/* 필터 본체 — 모바일은 토글, 데스크탑(md+)은 항상 표시 */}
        <div className={filtersOpen ? 'mt-3 md:mt-0' : 'hidden md:block'}>
          {domainsQuery.data ? (
            <ListingFilterPanel
