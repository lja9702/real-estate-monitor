...
export function ComplexDetailPage() {
  ...
  const { stat, rows, deals } = query.data
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 매물 목록 */}
      <section>
        <h2 className="mb-2 text-base font-semibold">매물 ({rows.length}건)</h2>
        {rows.length === 0 ? (
      ...
      {/* 실거래 목록 */}
      <section>
        <h2 className="mb-2 text-base font-semibold">실거래 최근 2년 ({deals.length}건)</h2>
        {deals.length === 0 ? (
          <p className="rounded-lg border p-6 text-center text-sm text-muted-foreground">
            실거래 내역 없음
          </p>
