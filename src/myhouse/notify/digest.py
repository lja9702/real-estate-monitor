...
def _complex_block(
    ...
    *,
    ...
    """(라인, 신규수, 가격변동수, 거래완료수) — 가격밴드 밖 매물은 제외.

    카운트는 (헤더 표기를 위해) 매물(article) 단위다. 신규는 표시할 때만 cluster 로 접는다.
    """
    if cr.error:
        if include_errors:
            return ([f"━━ {escape(cr.label)} ━━", f"⚠ 수집 실패: {escape(cr.error)}", ""], 0, 0, 0)
        return ([], 0, 0, 0)
    cdiff = cr.diff
    if cdiff is None:
        return ([], 0, 0, 0)
    ...
    if not (new_dtos or price_ops or removed_ops):
        return ([], 0, 0, 0)
    ...
    lines.append("")
    return (lines, len(new_dtos), len(price_ops), len(removed_ops))
...
def build_digest(
    ...
    *,
    ...
    body: list[str] = []
    n_new = n_price = n_removed = 0
    for cr in result.complexes:
        if only_complexes is not None and cr.complex_no not in only_complexes:
            continue
        lines, a, b, c = _complex_block(
            cr, result.starred_complexes, price_min, price_max, include_errors=include_errors
        )
        body.extend(lines)
        n_new += a
        n_price += b
