...
HELP = (
    ...
    "• <code>/band 7 12</code> — 정기 알림 관심 가격대(억) 설정(<code>/band</code>=보기, <code>/band off</code>=전체). "
    "🔥급매·빈 단지 첫 매물은 밴드와 무관하게 항상 알림\n"
    "• <code>/list</code> — 내가 추가한 단지 목록\n"
    "• <code>/untrack 1234</code> 또는 <code>/untrack 방배 삼호1차</code> — 내 추적에서 제거(번호·이름)\n"
